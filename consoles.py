...
class ITATopo(Topo):
	...
	host_externo = self.addHost('h9999', vcpu=cpumax, ip='8.8.8.8/32')
	gateway_cta = self.addHost('h1000', vcpu=cpumax, ip='161.24.0.1/16')

#	gateway_cta.cmd('ip r add default dev h1000-eth1')
#	host_externo.cmd('ip r add h1000 dev h9999-eth0')
	...
	self.addLink(gateway_cta, host_externo,
			   bw=20, delay='2s', loss=10, max_queue_size=1000, use_htb=True)
...
def addRoutes(net):
	...
	if h.name == 'h3':
	    h.cmd('ip r add default dev h3-eth1')
	if h.name == 'h6':
	    h.cmd('ip r add default dev h6-eth1')
